...
Int_t Term0Pdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const {
...
// Analytical integral
Double_t Term0Pdf::analyticalIntegral(Int_t code, const char* rangeName) const {
	assert(code == 1);
	...
	std::cout << "Error in Term0Pdf::analyticalIntegral" << std::endl;
	return 0;
}
